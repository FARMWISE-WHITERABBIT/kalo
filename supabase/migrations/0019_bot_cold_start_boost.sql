-- bot_burst v4: cold-start + kickoff-urgency boosts in market selection.
--
-- What changed and why: v3 weighted markets by heat × (1 + 24h volume),
-- which is self-reinforcing — a brand-new market has zero volume, so the
-- weighted sampler almost never picks it, so it never earns volume (observed
-- live after 0018 seeded 72 World Cup markets: minutes of healthy burst
-- activity concentrated in established books while 71 of 72 new markets sat
-- untouched). Two additive terms fix the cold start without touching any
-- engine semantics:
--   * new-market boost (+150 for 48h after creation) — bootstraps liquidity
--     in ANY newly listed market, including approved news proposals;
--   * kickoff-urgency boost (+400 from 2h before kickoff through 24h out)
--     — game markets heat up into kickoff the way real sports books do.
-- Everything else in the function is identical to v3 (0014). Bots still act
-- only through place_order/cancel_order; invariants see nothing new.
